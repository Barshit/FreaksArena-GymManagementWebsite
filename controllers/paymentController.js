...
async function listPayments(req, res) {
  try {
    ...
    console.error(
      `[Admin ${req.session.adminId}] Error listing payments:`,
      error,
    );
    return res
      .status(500)
      .json({ error: "Unable to list payments. Please try again later." });
  }
}
...
async function createPayment(req, res) {
  try {
    ...
    payload.memberName = memberExists.fullName;
    payload.memberPhone = memberExists.phone;
    ...
    console.error(
      `[Admin ${req.session.adminId}] Error creating payment:`,
      error,
    );
    return res
      .status(500)
      .json({ error: "Unable to create payment. Please try again later." });
  }
}
...
async function getPayment(req, res) {
  try {
    ...
    console.error(
      `[Admin ${req.session.adminId}] Error fetching payment ${req.params.paymentId}:`,
      error,
    );
    return res
      .status(500)
      .json({ error: "Unable to fetch payment. Please try again later." });
  }
}
...
payload.memberPhone = memberExists.phone;
    ...
    return res
      .status(500)
      .json({ error: "Unable to update payment. Please try again later." });
  }
}
...
async function deletePayment(req, res) {
  try {
    ...
    console.log(
      `[Admin ${adminId}] Error deleting payment ${req.params.paymentId}:`,
      error,
    );
    return res
      .status(500)
      .json({ error: "Unable to delete payment. Please try again later." });
  }
}
