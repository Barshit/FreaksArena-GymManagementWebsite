(function () {
  ...
  function getMemberName(memberId, members) {
    const member = members.find(
      (item) => String(item._id) === String(memberId),
    );
  ...
  async function renderPayments() {
    ...
    if (!payments.length) {
      tableBody.innerHTML =
        '<tr><td colspan="7" class="empty-row">No payment records are available yet.</td></tr>';
      return;
    }
    ...
    tableBody.innerHTML = payments
      .map((payment) => {
        ...
        const memberName =
          memberData.fullName || payment.memberName || "Unknown member";
  ...
  async function handlePaymentFormSubmit(event) {
    ...
    try {
      ...
      if (currentEditingPaymentId) {
        response = response = await csrfFetch(
          `/api/payments/${currentEditingPaymentId}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok) {
        ...
        if (errorEl) {
          errorEl.textContent =
            errorData.error || "Unable to save payment. Please try again.";
        }
        return;
      }
      ...
      if (values.membershipPlan === "Membership Renewal" && values.memberId) {
        try {
          ...
          if (!renewResponse.ok) {
            throw new Error(
              `Unable to renew membership (${renewResponse.status})`,
            );
          }
          ...
          window.alert(
            "Payment recorded, but the membership renewal could not be applied.",
          );
        }
      }
  ...
  async function openPaymentModal() {
    ...
    if (!membersCache.length) {
      window.alert(
        "No members are available. Add a member before recording a payment.",
      );
      return;
    }
    ...
    filterPaymentMembers(
      document.getElementById(PAYMENT_MEMBER_SEARCH_ID),
      form,
    );
  ...
  async function openPaymentModalForEdit(paymentId) {
    ...
    if (payment.member) {
      form.elements.memberId.value = String(payment.member._id);
    } else {
      window.alert(
        "This member has been deleted. This payment can still be viewed, but it is no longer linked to an active member.",
      );
  ...
  function setupPaymentsPage() {
    ...
    const closePaymentModalButton = document.getElementById(
      "close-payment-modal",
    );
